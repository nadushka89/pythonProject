...
def test_deploy(start_time):
    ...
    # Загрузка пакета p7zip на удаленную машину
    ...
    res.append(ssh_checkout(config['ssh_host'], config['username'], config['ssh_password'], cmd_install, "Настраивается пакет"))

    # Проверка установки на удаленной машине
    cmd_check_install = f"echo '{config['ssh_password']}' | sudo -S dpkg -s p7zip-full"
    res.append(ssh_checkout(config['ssh_host'], config['username'], config['ssh_password'], cmd_check_install, "Status: install ok installed"))

    assert all(res)
    end_time = datetime.datetime.now().strftime('%Y-%m-%d %H:%M:%S')
    save_journal_log(start_time, end_time, 'log_test_deploy_ssh.txt')
