...
def test_click_about(browser):
    ...
    sleep(3)
    assert testpage.about_success() == f"{testdata['about_title']}", "test_about_page FAILED"


def test_text_about(browser):
    logging.info("Test login_positive Starting")
    testpage = OperationsHelper(browser)
    testpage.go_to_site()
    testpage.enter_login(testdata["login"])
    ...
    testpage.click_about_button()
    sleep(3)
    font_size = testpage.get_about_title_font_size()
    assert font_size == testdata["font_size"], "test_about_page FAILED"
